...
class top extends Module{
    ...
    val imm_mux = MuxLookup(control_mod.io.extend_Sel,false.B, Array(
    ...
    aluControl_mod.io.func3 := io.instruction(14, 12)
    aluControl_mod.io.func7 := io.instruction(31, 25)
    ...
    // REG FILE MUX
    val readData1Mux = MuxLookup(control_mod.io.operand_A, false.B, Array(
    ...
    val branch_and = control_mod.io.branch & alu_mod.io.branch_out
    val branchAdd = regFile_mod.io.rs1.asSInt() + imm_mux.asSInt()
    val branchMux = Mux(branch_and.asBool(), immdGen_mod.io.sb_imm, pC_mod.io.pc4)
    val pc_mux = MuxLookup(control_mod.io.nextPc_Sel, false.B, Array(
    ("b00".U) -> pC_mod.io.pc4,
    ("b01".U) -> branchMux,
    ("b10".U) -> immdGen_mod.io.uj_imm,
    ("b11".U) -> branchAdd
    ))
    pC_mod.io.in := pc_mux.asSInt()
